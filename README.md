...
- Shader-backed wide-line rendering for molecular and interaction lines, including parallel double/triple bond lines when bond-order metadata is available.
...
The first launch starts with an empty viewer unless a previous server-side session exists. In that clean state, legacy `/api/last-structure` requests return `404 {"error":"not_found"}` rather than a server error. Use `Open file` to load a browser-local `pdb`, `cif`, `sdf`, `mol`, `mol2`, `xyz`, `mae`, `maegz`, or Maestro `psazip` file. Use `Open server` to browse structure files on the machine running `server.py` through a Linux-style explorer dialog with Places, breadcrumbs, Back/Forward/Up navigation, and file metadata; by default it is limited to the server user's home directory, and `--file-root <dir>` may be supplied one or more times to choose exposed roots. MAE/MAEGZ loading uses the bundled pure-Python converter and does not require a Schrodinger installation. Multi-CT MAE/MAEGZ files and multi-record SDF files are loaded as separate viewer entries rather than collapsed into one entry. The converter preserves Maestro bond order metadata for line rendering, while plain PDB `CONECT` inputs that do not encode bond order remain single-line. PSAZIP loading extracts the embedded structure and precomputed Maestro `.vis` surface without a Schrodinger runtime; when the PSAZIP includes Bioluminate patch pickle data, the surface is colored by the positive/negative electrostatic field using a red-white-blue ramp. The Python environment must have `h5py` and `numpy` for that surface parser. Each load gets a unique internal entry id, so loading the same filename again creates another entry while preserving the filename as the display title. Double-click an entry title in the Entries panel to rename that displayed title.
