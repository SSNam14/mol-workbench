...
- Right-clicking a selected Hierarchy row opens a small context menu. Its `Delete` action deletes from the current view by hiding the selected atoms; it must not remove the source entry or mutate the loaded structure data.
