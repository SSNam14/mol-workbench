...
- `Open server` browses server-side files under configured `--file-root` directories only. It uses a compact Linux-style explorer dialog with Places, breadcrumb navigation, Back/Forward/Up controls, and file metadata columns. It should list directories and supported molecular structure files, omit hidden path segments, reject paths outside the allowed roots, and load selected files through `/api/server-file-load` as new unique entries without editing the original source file. Entries loaded from the server filesystem preserve the absolute source path in `entry.sourcePath`; browser-local uploads and URL loads may not have a server-side source path.
...
- Interaction-specific agent behavior should be built from generic `queryInteractions` / `showInteractions` source-target selector queries over existing interaction indexes. The default index remains per-entry. Cross-entry indexes are allowed only for visible entries in the same multi-entry `loadGroup` where a protein-containing entry is paired with a non-protein-containing entry, which supports Glide-style receptor-first MAEGZ pose files without global all-vs-all interaction builds. These actions must filter only the interaction guide lines that match the query and default display units to residues. Manual interaction-panel type/scope edits clear any active agent interaction filter so the GUI can return to normal all/scope-controlled display.
...
- When multiple entries are displayed, render the ready interaction indexes for each visible entry plus eligible same-`loadGroup` receptor/ligand pair indexes. Index worker builds are queued to avoid starting several heavy builds at once.
...
- `N` expands the current selection to nearby atoms within 5A of the selected atoms. It remains entry-local for normal entries, but entries from the same multi-entry `loadGroup` may compare across entries when either side contains protein atoms. This supports Glide-style receptor-first MAEGZ files where the receptor and ligand poses are separate entries without mixing ligand-only pose bundles. Expansion follows the current top-left selection level (`Atoms`, `Residues`, `Chains`, or `Molecules`) and adds to the existing selection.
...
- The Clip control is a normalized UI over the viewer's 3Dmol slab offsets, not a direct Angstrom readout. Any `Z`/fit operation that rewrites `viewer.slabNear/Far` must store those values as the new baseline and reset the handles to the default 25% / 75% positions. Moving a handle maps that normalized position back onto the current baseline span, so the same handle position may correspond to different Angstrom slab offsets after a different fit. Do not show numeric clip values in the UI.
...
- `line` rendering is handled by `wide-lines.js`, not native WebGL line width. Protein atom lines, ligand lines, style-rule lines/tube side lines, selection line highlights, and interaction lines are converted to static segment/cap mesh geometry inside the 3Dmol scene, so they participate in depth testing. The vertex shader expands the geometry in screen space and scales width by camera depth/zoom, avoiding per-frame JavaScript projection/vertex rewrites. Current screen-pixel clamps keep molecular lines around 2-8 px. When bond-order metadata exists, molecular line rendering draws double and triple bonds as parallel line segments; formats that provide only unordered connectivity remain single-line. Dashed wide lines are for interaction guide rendering only, not molecular representation styling.
- MAE/MAEGZ conversion must preserve Maestro bond order metadata as entry `bondOrders` alongside the converted PDB text. This keeps double-bond line display available after conversion even though PDB `CONECT` itself cannot encode bond order. Multi-CT MAE/MAEGZ files and multi-record SDF files must load as separate viewer entries, not as one merged molecule.
